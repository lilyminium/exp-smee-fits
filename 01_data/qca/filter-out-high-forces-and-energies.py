"""
Filter out conformers with high RMS forces and high relative energies from QCArchive datasets.
...
exceeds a specified threshold or the relative energy exceeds a specified threshold.
High RMS forces typically indicate:
- Geometries far from equilibrium
- Poor optimization convergence
- Numerical instabilities in QM calculations

High relative energies typically indicate:
- Conformers far from the global minimum
- Unstable or high-energy structures
...
- Conformers with relative energy > threshold removed
...
And relative energy compared to lowest energy conformer:
...
@click.option(
    ...
    type=float,
    default=10.0,
    help="Energy threshold (in kcal/mol) above which conformers will be filtered out (relative to lowest energy conformer)"
)
def main(
    input_path: str,
    output_path: str,
    ...
    for entry in tqdm.tqdm(dataset, desc="Processing"):
        ...
        # Store conformers that pass the force and energy threshold filters
        new_coords = []
        new_forces = []
        new_energies = []
        ...
        for i, fc in enumerate(forces):
            ...
            # Keep conformer if energy and force are below threshold
            if force_rms <= force_threshold and energy_diff <= energy_threshold:
                new_coords.append(coords[i])
                new_forces.append(forces[i])
                new_energies.append(energies[i])
            else:
                # Filter out high-force or high-energy conformer and log it
                n_removed += 1
                logger.info(
                    f"Filtering conformer for {entry['smiles']} "
                    f"with RMS force {force_rms:.2f} > {force_threshold} or energy difference {energy_diff:.2f} > {energy_threshold}"
                )
